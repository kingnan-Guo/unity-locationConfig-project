...
public class cameraOperation : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        EventCenterOptimize.getInstance().AddEventListener<string>("changeCameraPosition", (data) => {
            Debug.Log("changeCameraPosition ="+ data);

            // transform.RotateAround(LookAtCube.transform.position, Vector3.right, 20);
            count = 0;

            
            // transform.RotateAround(LookAtCube.transform.position, Vector3.up, -90);
            // LookAtCube.transform.Rotate(Vector3.up, -90);
