...
public class DragTo3D : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerClickHandler
{
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        if (Time.time - lastClickTime < doubleClickInterval)
        {
            ...
            foreach (var items in results)
            {
                if(items.gameObject.CompareTag(gloab_TagName.CANVAS_DEVICE_DISABLE)){
                    ...
                    FindDevicePositionInTreeDScene(items.gameObject.name);
                }
            }
        }
        else
        {
            ...
            foreach (var items in results)
            {
                Debug.Log("SingleClick gameObject =="+items.gameObject.name + " == "+ items.gameObject.tag);
    ...
    public void FindDevicePositionInTreeDScene(string deviceName){
       ...
       Transform mainMap =  GameObject.FindGameObjectWithTag(gloab_TagName.MAIN_MAP).transform;
        ...
        Transform deviceTransform = globalUtils.getInstance().FindGameObjectRecursive(mainMap, deviceName)?.transform;
        if(!deviceTransform){
            return;
        }
        // GameObject.FindWithTag(gloab_TagName.CANVAS_DEVICE);
        
        // Debug.Log("GameObject aa =="+deviceTransform);
        // Debug.Log("GameObject =="+deviceTransform.parent.name+" =="+deviceTransform.parent.gameObject.activeInHierarchy);
    
        
        if(deviceTransform.parent.gameObject.activeInHierarchy){
            // if(string.Equals(aa.parent.name, "CanvasDevice")){
            // }
            // 通知建筑物 展示  当前点位 所在的楼层

            if(deviceTransform.parent.tag == gloab_TagName.MAIN_MAP){
            ...
            } else if(deviceTransform.parent.tag == gloab_TagName.BUILDING){
                // aa.parent.name.Split("_")[0];

                
                // 模拟 点击 楼幢按钮
                buildingCardView.getInstance().setActiveBuilding(deviceTransform.parent.name.Split("_")[0]);
                EventCenterOptimize.getInstance().EventTrigger<string>(gloab_EventCenter_Name.CHANGE_MAIN_BUILDING, deviceTransform.parent.name.Split("_")[0]);
                ...
                Transform transform = GameObject.FindGameObjectWithTag(gloab_TagName.MAIN_CANVAS).transform.Find("floorCard/box/F "+ deviceTransform.parent.name).transform;
                buildingCardView.getInstance().setButtonActive<Transform>(transform);
                EventCenterOptimize.getInstance().EventTrigger<string>(gloab_EventCenter_Name.SHOW_APPOINT_FLOOR, deviceTransform.parent.name);
            }
        ...
        globalUtils.getInstance().changeCameraPosition(deviceTransform, 20);
    ...
    public void DragModel(){
        ...
        if(Input.GetMouseButton(0)){
            if(Mathf.Abs(Input.GetAxis("Mouse X")) > 1f && gameObject != null){
                // Debug.Log("检测鼠标横向运动" + Input.GetAxis("Mouse X"));
                Debug.Log("拖动了设备");
                scrolling = true;
            }
            if(gameObject != null){
