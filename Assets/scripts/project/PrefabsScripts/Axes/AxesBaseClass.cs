...
public class AxesBaseClass : MonoBehaviour, IObservable
{
    /// <summary>
    /// 要忽略的图层
    /// </summary>
    ...
    private List<System.Action> _observers = new List<System.Action>();
    void Start()
    {
    ...
    /// 完成操作
    /// </summary>
    protected virtual void Done(Transform transform = null) {
        Debug.Log("Done " + currentAxisParent);
        NotifyObservers(currentAxisParent);
    }


    public void Subscribe(System.Action onChanged)
    {
        _observers.Add(onChanged);
    }
 
    public void Unsubscribe(System.Action onChanged)
    {
        _observers.Remove(onChanged);
    }

    private void NotifyObservers(Transform currentAxisParent)
    {
    ...
    private void RemoveAll(){
        foreach (var observer in _observers)
        {
            ...
            Debug.Log("RemoveAll " + observer);
        }
    }
    // 销毁
    void OnDestroy(){
        Debug.Log("AxesBaseClass Destroy");
        RemoveAll();
    }
