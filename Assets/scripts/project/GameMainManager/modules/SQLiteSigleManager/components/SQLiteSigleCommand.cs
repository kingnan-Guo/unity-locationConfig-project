...
public class SQLiteSigleCommand : baseManager<SQLiteSigleCommand>
{
    ...
    /// <returns></returns>
    public int DeleteByIMEI<T>(List<string> imeiList) where T : BaseDataClass
    {
        ...
        foreach (string imei in imeiList)
        {
            ...
            string sql = $"IMEI = '{imei}'";
            // DeleteBySql<T>(sql);
            Count += DeleteBySql<T>(sql);
        }
        return Count;
    }
    /// <summary>
    ...
    public int DeleteBySql<T>(string sqlFliter) where T : BaseDataClass{
        return SQLiteDBCommand.DeleteBySql<T>(sqlFliter);
    }
    ...
    public void DeleteDatabase(string DatabaseName){
        // SQLiteDBCommand.DeleteDatabase(Name);

        // 删除文件
        // string path = Application.persistentDataPath + "/" + Name + ".db";
        SQLiteDBCommand.DeleteDatabase(DatabaseName);
        // string filePath = Path.Combine(Application.streamingAssetsPath, $"SQLiteDatabase/{DatabaseName}.db");
